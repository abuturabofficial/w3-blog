...
> [!TIP] 
> You have learnt only about the few basic terms, that are helpful to understand the Linux directory structure. The detailed guide will be covered in some other blog or watch [this video](https://www.youtube.com/watch?v=p9lCbFq8IPo).
...
It contains all the files necessary for system boot up including Linux Kernel. You can install more than one Kernel inside your `/boot` in case one fails.

![Omarchy (Arch Linux) /boot](linux-directories-3.webp)

## Root Home Directory `/root`

The `/root` is a home for root user. It contains all the files, directories, and configuration related to the root user, i.e., root `.bashrc` etc.
